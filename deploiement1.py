...
# Remplacer le graphique Seaborn par Matplotlib
fig, ax = plt.subplots()
train_df[column].value_counts().plot(kind='bar', ax=ax, color='skyblue')
ax.set_title(f'Distribution de {column}')
ax.set_xlabel(column)
ax.set_ylabel('Count')
st.pyplot(fig)

# Matrice de corrélation avec Matplotlib
st.subheader("Matrice de Corrélation")
cor = train_df.corr()
fig, ax = plt.subplots(figsize=(10, 8))
cax = ax.matshow(cor, cmap='coolwarm')
fig.colorbar(cax)
